...
final class FrameworkRenderState
{
    ...
    public static function isFirstTypeCall(string $type, ComponentChannel $channel): bool
    {
        return !isset(self::$alreadyCalledTypes[self::typeKey($type, $channel)]);
    }

    public static function markTypeCalled(string $type, ComponentChannel $channel): void
    {
        self::$alreadyCalledTypes[self::typeKey($type, $channel)] = true;
    }

    public static function isFirstComponentCall(string $componentName, ComponentChannel $channel, string $style): bool
    {
        return !isset(self::$alreadyCalledComponents[self::componentKey($componentName, $channel, $style)]);
    }

    public static function markComponentCalled(string $componentName, ComponentChannel $channel, string $style): void
    {
    ...
    private static function typeKey(string $type, ComponentChannel $channel): string
    {
        return $type.$channel->value;
    }

    private static function componentKey(string $componentName, ComponentChannel $channel, string $style): string
    {
