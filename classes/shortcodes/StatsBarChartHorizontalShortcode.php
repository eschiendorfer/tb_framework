...
class StatsBarChartHorizontalShortcode implements \ShortcodeModule\ShortcodeInterface, \ShortcodeModule\ShortcodeEditorUploadInterface
{
    public static function getName(): string
    {
        return 'stats_bar_chart_horizontal';
    }
    ...
    public static function getEditorFields(): array
    {
        return [
            ...
            ComponentEditorContractHelper::styleField(new StatsBarChartHorizontalComponent()),
        ];
    }
    ...
    public static function handleEditorUpload(string $fieldKey, array $file): array
    {
        ...
        $fileName = ComponentJsonDataLoader::saveUploadedFile(new StatsBarChartHorizontalComponent(), $file);
    ...
    public function render(array $params, string $channel, array $context = []): string
    {
        ...
        $component = new StatsBarChartHorizontalComponent();
        $data = ComponentJsonDataLoader::loadFromParams($params, $component);
        if (empty($data)) {
            return '';
        }

        return StatsBarChartHorizontalComponent::fetchWeb($data, trim((string)($params['style'] ?? '')));
    }
}
